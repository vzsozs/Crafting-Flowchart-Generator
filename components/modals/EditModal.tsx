...
import { CustomNode, MachineNodeData, ResourceNodeData, RecipeItem } from '../../types';
...
const EditModal: React.FC<EditModalProps> = ({ node, onClose, onSave }) => {
  ...
  const handleRecipeChange = (
    ...
    if (field === 'amount') {
        newList[index] = { ...newList[index], [field]: parseFloat(value as string) || 0 };
    } else {
  ...
  const addRecipeItem = (list: 'inputs' | 'outputs') => {
    const machineData = data as MachineNodeData;
    const newItem: RecipeItem = { id: `new_${list}_${Date.now()}`, name: 'New Item', amount: 1 };
        ...
        <div>
            <div className="mb-4">
                ...
                <input
                    ...
                    className="w-full p-2 bg-gray-900 border border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500"
                />
            </div>
            ...
            <div className="mb-4">
                ...
                {machineData.inputs.map((item, index) => (
                    <div key={item.id || index} className="grid grid-cols-12 gap-2 mb-2 items-center">
                        <input type="text" value={item.name} onChange={(e) => handleRecipeChange('inputs', index, 'name', e.target.value)} className="col-span-7 p-2 bg-gray-900 border border-gray-600 rounded-md text-sm" />
            ...
            <div>
                ...
                {machineData.outputs.map((item, index) => (
                    <div key={item.id || index} className="grid grid-cols-12 gap-2 mb-2 items-center">
                        <input type="text" value={item.name} onChange={(e) => handleRecipeChange('outputs', index, 'name', e.target.value)} className="col-span-7 p-2 bg-gray-900 border border-gray-600 rounded-md text-sm" />
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
      <div className="bg-gray-800 text-white rounded-lg shadow-2xl p-6 w-full max-w-2xl max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center border-b border-gray-600 pb-3 mb-4">
            <h3 className="text-2xl font-bold text-teal-300">Edit Node: <span className="text-white">{node.type === 'machine' ? (data as MachineNodeData).label : (data as ResourceNodeData).resource}</span></h3>
