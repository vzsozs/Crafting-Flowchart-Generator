...
function App() {
  ...
  useEffect(() => {
    ...
    nodes.forEach(node => {
      if (node.type === 'machine') {
        ...
        data.outputs.forEach(output => { nameSet.add(output.name); const current = totals.get(output.name) || { totalInput: 0, totalOutput: 0 }; current.totalOutput += output.amount * opsPerSec; totals.set(output.name, current); });
  ...
  const onNodeClick = useCallback((event: React.MouseEvent, node: Node) => {
    ...
    if (action === 'edit') {
      ...
      addMachineFromResource(node.id);
    }
  ...
  const handleUpdateNode = (updatedData: MachineNodeData | ResourceNodeData) => {
    ...
    if (editingNode.type === 'machine') {
      ...
      const resourceNodesToUpdate = new Map<string, { resource: string }>();
          ...
          newMachineData.outputs.forEach(newOutput => {
            ...
            if (oldOutput && oldOutput.name !== newOutput.name) {
              ...
              if (edge) {
                resourceNodesToUpdate.set(edge.target, { resource: newOutput.name });
              }
            }
        });
      ...
      setNodes(nds => 
        nds
          .map(n => {
            // A szerkesztett gép frissítése
            if (n.id === editingNode.id) return { ...n, data: updatedData };
            // Az átnevezett resource node-ok frissítése
            if (resourceNodesToUpdate.has(n.id)) {
              return { ...n, data: { ...n.data, ...resourceNodesToUpdate.get(n.id) } };
            }
            return n;
          })
      ...
      setEdges(eds => 
        eds
          .map(e => edgesToUpdate.find(ue => ue.id === e.id) || e)
      ...
      setNodes(currentNodes => 
        currentNodes.map(node => {
          if (node.id === editingNode.id) {
            return { ...node, data: newResourceData };
          }
    ...
    <div className="flex h-screen w-screen bg-gray-800 text-white">
      ...
      <main className="flex-grow h-screen">
        <FlowCanvas 
          ...
          onConnect={onConnect} 
          onNodeDoubleClick={(_, node) => openEditModal(node)}
      ...
      {editingNode && 
        <EditModal 
          ...
          onAddMachine={editingNode.type === 'resource' ? () => addMachineFromResource(editingNode.id) : undefined}
        />
      }
    </div>
  );
